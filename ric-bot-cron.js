...
(async function () {
                ...
                try {
                    ...
                    function getTokenPriceInWRIC(pair, token) {
                        if (pair.token0 === WRIC && pair.token1 === token) {
                            ...
                            const reserveToken = normalizeToEther(pair.reserve1, 18);
                            if (reserveToken.isZero()) return ethers.BigNumber.from(0);
                            return reserveWRIC.mul(ethers.BigNumber.from(10).pow(18)).div(reserveToken);
                        } else if (pair.token0 === token && pair.token1 === WRIC) {
                            const reserveToken = normalizeToEther(pair.reserve0, 18);
                    ...
                    if (pairAB.token0 === tokenA && pairAB.token1 === tokenB) {
                        const reserveA = normalizeToEther(pairAB.reserve0, 18);
                        const reserveB = normalizeToEther(pairAB.reserve1, 18);
                        if (reserveA.isZero()) return null;
                        ratioAB = reserveB.mul(ethers.BigNumber.from(10).pow(18)).div(reserveA);
                    } else if (pairAB.token0 === tokenB && pairAB.token1 === tokenA) {
                        const reserveB = normalizeToEther(pairAB.reserve0, 18);
                        const reserveA = normalizeToEther(pairAB.reserve1, 18);
                    ...
                    const priceA_viaB = priceB.mul(ratioAB).div(ethers.BigNumber.from(10).pow(18));

                    const diff = priceA_viaB.sub(priceA_direct);
                    const percentDiff = diff.mul(10000).div(priceA_direct).toNumber() / 100;
            ...
            // ==================== FIND BEST AMOUNT - TANPA FILTER ====================
            async function findBestAmount(opp, maxNative) {
                try {
                    // Test berbagai ukuran modal untuk cari profit maksimal
                    const testFractions = [0.01, 0.03, 0.05, 0.10, 0.20, 0.35, 0.50, 0.70, 1.0];
                    let bestAmount = ethers.BigNumber.from(0);
                    let bestProfit = ethers.BigNumber.from(0);

                    for (let fraction of testFractions) {
                        const testAmount = maxNative.mul(Math.floor(fraction * 100)).div(100);
                        if (testAmount.isZero() || testAmount.lt(ethers.utils.parseEther("0.00001"))) continue;

                        const result = await simulateOpportunity(opp, testAmount);
                        if (result.isZero()) continue;

                        const profit = result.sub(testAmount);
                    ...
                    // TIDAK ADA FILTER PROFIT MINIMUM
                    // Ambil semua yang profit > 0
                    if (bestProfit.lte(0)) {
                        return { amount: ethers.BigNumber.from(0), profit: ethers.BigNumber.from(0) };
                    }

                    log(`🔍 Best amount: ${formatEther(bestAmount)} WRIC (profit: ${formatEther(bestProfit)} WRIC)`, 'DEBUG');
            ...
            async function swapOnDex(dex, amountIn, tokenIn, tokenOut, hopIndex, totalHops) {
                try {
                    ...
                    // amountOutMin = amountOut (SLIPPAGE 0)
            ...
            async function executeOpportunity(opp, amountWRIC, expectedProfit) {
                try {
                    ...
                    if (opp.hopCount === 2) estimatedGasAmount = CONFIG.gasEstimate2Hop;
                    else if (opp.hopCount === 3) estimatedGasAmount = CONFIG.gasEstimate3Hop;
                    else estimatedGasAmount = CONFIG.gasEstimate4Hop;

                    // TIDAK ADA GAS BUFFER
            ...
            async function mainLoop() {
                ...
                log("⚡ SLIPPAGE 0% - GAS 0.1 GWEI - NO FILTERS");
                log("🎯 Ambil SEMUA peluang sekecil apapun!");
                ...
                try {
                    ...
                    for (let opp of opportunities) {
                        try {
                            ...
                            const { amount: bestAmount, profit: bestProfit } = await findBestAmount(opp, maxUsable);

                            if (bestAmount.gt(0) && bestProfit.gt(0)) {
                                evaluated.push({ opp, amount: bestAmount, profit: bestProfit });
                            }
                        } catch (e) {
                    ...
                    evaluated.sort((a, b) => b.profit.gt(a.profit) ? 1 : -1);

                    let executed = false;
                    for (let { opp, amount, profit } of evaluated.slice(0, 3)) {
                        log(`🎯 Mencoba: ${opp.description}`);
                        log(`   💰 Modal: ${formatEther(amount)} WRIC`);
                        log(`   📈 Expected profit: ${formatEther(profit)} WRIC`);
